...
actual class FunSpecWrapper internal constructor(private val actual: FunSpec) {
    ...
    actual val returnType: TypeNameWrapper?
        get() = actual.returnType.let {
            when (it) {
                ...
                else -> object : TypeNameWrapper(it) {
                    ...
                    override fun copy(nullable: Boolean): TypeNameWrapper =
                        it.copy(nullable).let { copied ->
                            when (copied) {
                                ...
                                is com.squareup.kotlinpoet.ParameterizedTypeName -> ParameterizedTypeNameWrapper(
                                    copied
                                )
    ...
    actual fun toBuilder(name: String): FunSpecBuilderWrapper =
        FunSpecBuilderWrapper(actual.toBuilder(name))
...
actual class FunSpecBuilderWrapper internal constructor(private val actual: FunSpec.Builder) {
    ...
    actual fun addModifiers(vararg modifiers: KModifierWrapper): FunSpecBuilderWrapper =
        FunSpecBuilderWrapper(actual.addModifiers(*modifiers.map { it.toKotlinPoet() }
            .toTypedArray()))
